...
StdMat2 *NifImporter::ImportMaterialAndTextures(ImpNode *node, NiAVObjectRef avObject)
{
	...
	vector<NiPropertyRef> bsprops;
	for (int i = 0; i < 2; i++)
	{
		bsprops.push_back((DynamicCast<NiTriBasedGeom>(avObject))->GetBSProperty(i));
	}

	NiMaterialPropertyRef matRef = SelectFirstObjectOfType<NiMaterialProperty>(props);

	BSShaderPropertyRef shaderRef = SelectFirstObjectOfType<BSShaderProperty>(bsprops);
	BSLightingShaderPropertyRef lightingShaderRef = SelectFirstObjectOfType<BSLightingShaderProperty>(bsprops);
...
bool NifImporter::ImportNiftoolsShader(ImpNode *node, NiAVObjectRef avObject, StdMat2 *mtl)
{
   ...
   if ( BSLightingShaderPropertyRef lightingShaderRef = SelectFirstObjectOfType<BSLightingShaderProperty>(bsprops) )
   {
      // Material like properties
      {
         ...
         float shininess = 10; // lightingShaderRef->GetGlossiness();
